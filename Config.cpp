...
static size_t parseIndex(const std::string& text, const std::string& context)
{
    ...
    try { return static_cast<size_t>(std::stoull(text)); }
    catch (...) { throw std::runtime_error(context + " has an invalid index."); }
}
...
{
    if (!configPath.is_absolute()) throw std::runtime_error("The text configuration path must be absolute.");
    Config config;
    ...
    if (!stream) throw std::runtime_error("Could not open text configuration: " + pathToUtf8(config.configPath));
    ...
    while (std::getline(stream, line))
    {
        ...
        if (config.title.empty()) { config.title = text; continue; }
        ...
        if (text == "ENGINES") { section = Section::Engines; currentGroup = nullptr; continue; }
        if (text == "FILES") { section = Section::Files; currentGroup = nullptr; continue; }
        if (text.rfind("PROCESSES - ", 0) == 0)
        {
            if (config.groups.size() >= GroupCount) throw std::runtime_error("Configuration can contain at most two process groups.");
            const auto name = trim(text.substr(12));
            if (name.empty()) throw std::runtime_error("A process group name must not be empty.");
        ...
        if (section == Section::Engines)
        {
            ...
            if (config.engines.contains(values[0])) throw std::runtime_error(context + " contains duplicate engine '" + values[0] + "'.");
        ...
        {
            if (values.size() != 2) throw std::runtime_error(context + " must contain: Index | File.");
            const auto index = parseIndex(values[0], context);
            if (index != config.files.size()) throw std::runtime_error(context + " index must be " + std::to_string(config.files.size()) + ".");
        ...
        else if (section == Section::Processes)
        {
            ...
            const auto index = parseIndex(values[0], context);
            if (index != currentGroup->processes.size()) throw std::runtime_error(context + " index must be " + std::to_string(currentGroup->processes.size()) + ".");
            currentGroup->processes.emplace_back(ProcessConfig{.engineName = values[1], .commandArguments = values[2]});
        }
    }

    if (config.runsPerFile == 0) throw std::runtime_error("Configuration is missing RUNS.");
    if (config.engines.empty()) throw std::runtime_error("ENGINES must contain at least one engine.");
    if (config.files.empty()) throw std::runtime_error("FILES must contain at least one file.");
    if (config.groups.size() != GroupCount) throw std::runtime_error("Configuration must contain exactly two process groups.");
    if (config.groups[0].name == config.groups[1].name) throw std::runtime_error("Process group names must be unique.");
...
InputPlan buildInputPlan(const Config& config)
{
    ...
    for (size_t index = 0; index < config.files.size(); ++index)
    {
        ...
        file.relativeSource = file.source.filename();
