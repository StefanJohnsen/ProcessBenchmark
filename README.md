...
MEASURE TIME       : true
MEASURE RAM        : true
CREATE REPORT      : true  (creates .md report with the same name as config file)
```
...
```bash
./ProcessBenchmark "/home/user/Benchmark/processTest.txt"
```
...
MARKDOWN FILE : /full/path/processTest.md
```
...
the RAM counter maintained by the operating system kernel, so measurement overhead is minimal. Using 3 to 5 runs and
comparing medians helps smooth out normal variation from the OS and background activity.

### Measurement details

Run time uses `std::chrono::steady_clock`. RAM is the direct process's peak resident memory — `PeakWorkingSetSize` on
Windows, `ru_maxrss` on Linux; child-process memory and GPU/VRAM are excluded. Per-file comparisons use the median.
Use `--time-only` or `--ram-only` for one metric.

### Platform memory sources

**Windows** — the memory implementation follows Microsoft's Windows API documentation:
...
**Linux** — the memory implementation reads:

- [`wait4(2)`](https://man7.org/linux/man-pages/man2/wait4.2.html) / [`getrusage(2)`](https://man7.org/linux/man-pages/man2/getrusage.2.html)
  `ru_maxrss` supplies the process's peak resident set size at exit; no polling is required.
- [`/proc/cpuinfo`](https://man7.org/linux/man-pages/man5/proc_cpuinfo.5.html) supplies CPU name, vendor and
  physical/logical core counts.
- [`/proc/meminfo`](https://man7.org/linux/man-pages/man5/proc_meminfo.5.html) supplies usable and available physical
  memory and the commit limit (reported as the page-file limit). Installed (SMBIOS) memory capacity is not exposed to
  unprivileged processes on Linux, so it is reported as `N/A`.
...
> fictional. The same example is available as [`config.example.md`](config.example.md) in this repository. This
> example was generated on Windows; a report generated on Linux shows the same layout, with the "Benchmark Hardware"
> and "Memory Measurement" sections citing the Linux sources described above instead of the Windows APIs.**
